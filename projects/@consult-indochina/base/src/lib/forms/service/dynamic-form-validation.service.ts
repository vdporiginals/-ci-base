...
export class DynamicFormValidationService {
  constructor(
    @Optional() @Inject(NG_VALIDATORS) private _NG_VALIDATORS: ValidatorFn[],
    @Optional()
    @Inject(NG_ASYNC_VALIDATORS)
    private _NG_ASYNC_VALIDATORS: AsyncValidatorFn[],
    @Optional()
    @Inject(DYNAMIC_VALIDATORS)
    private _DYNAMIC_VALIDATORS: Map<string, Validator | ValidatorFactory>,
    @Optional()
    @Inject(DYNAMIC_ERROR_MESSAGES_MATCHER)
    private _DYNAMIC_ERROR_MESSAGES_MATCHER: DynamicErrorMessagesMatcher
  ) {}

  private getValidatorFn(
    validatorName: string,
    validatorArgs: any = null,
    validatorsToken: ValidatorsToken = this._NG_VALIDATORS
  ): Validator | never {
  ...
  private getValidatorFns(
    validatorsConfig: DynamicValidatorsConfig,
    validatorsToken: ValidatorsToken = this._NG_VALIDATORS
  ): Validator[] {
    ...
    if (isObject(validatorsConfig)) {
      validatorFns = Object.keys(validatorsConfig).map((validatorConfigKey) => {
        const validatorConfigValue = (validatorsConfig as DynamicValidatorsConfig)[
          validatorConfigKey
        ];
  ...
  getAsyncValidators(
    asyncValidatorsConfig: DynamicValidatorsConfig
  ): AsyncValidatorFn[] {
  ...
  updateValidators(
    validatorsConfig: DynamicValidatorsConfig | null,
  ...
  updateAsyncValidators(
    asyncValidatorsConfig: DynamicValidatorsConfig | null,
    control: AbstractControl,
    model: DynamicFormControlModel
  ): void {
    ...
    if (asyncValidatorsConfig === null) {
      ...
      control.setAsyncValidators(
        this.getAsyncValidators(asyncValidatorsConfig)
      );
    }
  ...
  showErrorMessages(
    ...
    const matcher = this._DYNAMIC_ERROR_MESSAGES_MATCHER
      ? this._DYNAMIC_ERROR_MESSAGES_MATCHER(control, model, hasFocus)
      : DEFAULT_ERROR_STATE_MATCHER(control, model, hasFocus);

    return precondition && matcher;
  }

  parseErrorMessageConfig(
    template: string,
    model: DynamicFormControlModel,
    error: any = null
  ): string {
    return template.replace(
      ...
      (_match: string, expression: string) => {
        ...
        return propertySource[propertyName] !== null &&
          propertySource[propertyName] !== undefined
  ...
  isFormHook(value: any): boolean {
    return (
      isString(value) &&
      (Object.values(DynamicFormHook) as string[]).includes(value)
    );
  }
