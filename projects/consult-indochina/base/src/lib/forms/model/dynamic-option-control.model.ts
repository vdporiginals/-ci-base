...
export interface DynamicOptionControlModelConfig<T>
  ...
  options?:
    | DynamicFormOptionConfig<T>[]
    | Observable<DynamicFormOptionConfig<T>[]>;
}

export abstract class DynamicOptionControlModel<
  T
> extends DynamicFormValueControlModel<T | T[]> {
  @serializable('options') private _options: DynamicFormOption<T>[] = [];
  options$!: Observable<DynamicFormOption<T>[]>;

  protected constructor(
    config: DynamicOptionControlModelConfig<T>,
  ...
  set options(options: any) {
    if (Array.isArray(options)) {
      this._options = (options as DynamicFormOptionConfig<T>[]).map(
        (optionConfig) => new DynamicFormOption<T>(optionConfig)
      );

      this.updateOptions$();
    } else if (isObservable(options)) {
      this.options$ = (options as Observable<
        DynamicFormOptionConfig<T>[]
      >).pipe(
        map((optionsConfig) => {
          this._options = optionsConfig.map(
            (optionConfig) => new DynamicFormOption<T>(optionConfig)
          );
  ...
  insert(
    index: number,
    optionConfig: DynamicFormOptionConfig<T>
  ): DynamicFormOption<T> {
