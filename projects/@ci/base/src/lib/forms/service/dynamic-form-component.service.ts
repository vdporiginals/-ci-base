...
export type DynamicFormControlMapFn = (
  model: DynamicFormControlModel
) => Type<DynamicFormControl> | null;

export const DYNAMIC_FORM_CONTROL_MAP_FN = new InjectionToken<DynamicFormControlMapFn>(
...
export class DynamicFormComponentService {
  ...
  private formControls: {
    [key: string]: DynamicFormControlRef | DynamicFormControlRef[];
  } = {};

  constructor(
    @Inject(DYNAMIC_FORM_CONTROL_MAP_FN)
    @Optional()
    private readonly DYNAMIC_FORM_CONTROL_MAP_FN: any
  ) {
  ...
  getFormControlRef(
    modelId: string,
    index?: number
  ): DynamicFormControlRef | undefined {
    const ref: DynamicFormControlRef | DynamicFormControlRef[] = this
      .formControls[modelId];
  ...
  registerFormControl(
    ...
    if (isNumber(index)) {
      ...
      const arrayRef: DynamicFormControlRef[] =
        (this.formControls[model.id] as DynamicFormControlRef[]) || [];
      ...
      if (Array.isArray(arrayRef)) {
        ...
        console.warn(
          `registerFormControlRef is called with index for a non-array form control: ${model.id}`
        );
      }
    } else {
  ...
  getCustomComponentType(
    ...
    return isFunction(this.DYNAMIC_FORM_CONTROL_MAP_FN)
      ? this.DYNAMIC_FORM_CONTROL_MAP_FN(model)
      : null;
  }
}
