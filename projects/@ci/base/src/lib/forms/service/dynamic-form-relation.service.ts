...
export class DynamicFormRelationService {
  constructor(
    @Optional()
    @Inject(DYNAMIC_MATCHERS)
    private MATCHERS: DynamicFormControlMatcher[],
  ...
  getRelatedFormControls(
    ...
    const conditionReducer = (
      ...
      if (!controls.hasOwnProperty(path)) {
        const control = condition.rootPath
          ? group.root.get(condition.rootPath)
          : group.get(condition.id);

        control instanceof FormControl
          ? (controls[path as any] = control)
          : console.warn(
              `No related form control with id ${condition.id} could be found`
            );
      }
    ...
    const relationReducer = (controls: any, relation: { when: any[] }) =>
      relation.when.reduce(conditionReducer, controls);

    return model.relations.reduce(relationReducer, {});
  }

  findRelationByMatcher(
    relations: DynamicFormControlRelation[],
    matcher: DynamicFormControlMatcher
  ): DynamicFormControlRelation | undefined {
    return relations.find((relation) =>
      [matcher.match, matcher.opposingMatch].includes(relation.match)
    );
  }
  ...
  matchesCondition(
    ...
    return relation.when.reduce((hasAlreadyMatched, condition, index) => {
      ...
      if (relatedFormControl && relation.match === matcher.match) {
        ...
        return (
          condition.value === relatedFormControl.value ||
          condition.status === relatedFormControl.status
        );
      }
  ...
  subscribeRelations(
    ...
    Object.values(relatedFormControls).forEach((relatedControl) => {
      const valueChanges = relatedControl.valueChanges.pipe(
        startWith(relatedControl.value)
      );
      const statusChanges = relatedControl.statusChanges.pipe(
        startWith(relatedControl.status)
      );

      subscriptions.push(
        merge(valueChanges, statusChanges).subscribe(() => {
          this.MATCHERS.forEach((matcher) => {
            const relation = this.findRelationByMatcher(
              model.relations,
              matcher
            );

            if (relation !== undefined) {
              const hasMatch = this.matchesCondition(
                relation,
                relatedFormControls,
                matcher
              );
